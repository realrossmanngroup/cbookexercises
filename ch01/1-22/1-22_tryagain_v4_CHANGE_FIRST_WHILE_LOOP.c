...
int main()
{
    ...
    for (i = 0; i <= endofinput + 1; i++)
    {
        ...
        if (s[i] == '\0')
        {
            printf("s[%d] is a null terminator! DEBUGOUTPUT\n", i);
        }
    }

    printf("endofinput is: %d and LINELENGTH is %d DEBUGOUTOUT \n\n", endofinput, LINELENGTH);
    ...
    d) take s[0] to s[wheretowrap] and put this into a char array called w1, which will be the first line.
        ...
        printf("first while loop before wordwrap runs on string S which is %s DEBUGOUTPUT\n\n", s); /*for debugging*/
        wordwrap(s);
    }
...
{
    printf("wordwrap function ran DEBUGOUTPUT\n\n"); /*for debugging*/
    ...
    printf("b[%d] inside the function is %c DEBUGOUTPUT\n\n", x, b[x]); /*for debugging*/
    while (b[x] != EOF)
    {
        x--;                             /* count backwards*/

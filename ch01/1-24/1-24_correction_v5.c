...
void trackchars(char openchar, char closechar, int ifthisis0youclosedit[],
                int *counter, int *closecounter, int *maxcount, int opencolumn[], int openline[],
                int closecolumn[], int closeline[], int extraclosedtrackingarray[], int *maxclosed)
{
    ...
    if (program[line][column] == openchar) /*if we have an open character*/
    {
        ...
        printf("DEBUG %d OPEN %c FOUND: *counter is %d, ifthisis0youclosedit[%d] is %d, program[%d][%d] is %c \n\n",
               __LINE__, openchar, *counter, *counter, ifthisis0youclosedit[*counter], openline[*counter], opencolumn[*counter],
    ...
    else if (program[line][column] == closechar) /*if we have a closed char */
    {
        ...
        printf("DEBUG %d CLOSED %c FOUND: *counter is %d, ifthisis0youclosedit[%d] is %d, extraclosedtrackingarray[%d] is %d, program[%d][%d] is %c \n\n",
               __LINE__, closechar, *counter, *counter, ifthisis0youclosedit[*counter], *closecounter, extraclosedtrackingarray[abs(*closecounter)], closeline[*counter], closecolumn[*counter],
    ...
    else
    {
        ...
        // return OUT; /*end this function*/
    }
}

// FUNCTION TO PRINT THE LOCATION OF THE ERRORS IN SYNTAX OF CODE AFTER FINDING THEM

void printerrors(int ifthisis0youclosedit[], int maxcount, int openline[], int opencolumn[],
...
void main()
{
    ...
    for (line = 0; line < linecountinput; line++) /* go through each line of the program and iterate upwards once at the end */
    {
        ...
        for (column = 0; column < howlongisthisline[line]; column++)
        {
            ...
            if ((weare__acomment == OUT) && (areweinastring == OUT))
            {
                ...
                trackchars('(', ')', parenthesis, &parencount, &parenclosecount, &maxparenthesis, location_parenopen_column,
    ...
    */
    ...
    /*
    THIS IS THE NON-FUNCTION VERSION OF THE CODE
    THAT WILL PRINT OUT THE AREA NEAR WHERE YOUR ERROR WAS DETECTED! THIS WORKS!

        for (int whichp = 0; whichp <= maxparenthesis; whichp++)
        {
            if (parenthesis[whichp] != 0)
            {
                printf("line with mismatched parenthesis is probably the %dth parenthesis on line %d: issue includes this text on line %d: \n", whichp + 1, location_parenopen_line[whichp] + 1, location_parenopen_line[whichp] + 1);
                printf("%d debugoutput parenthesis[%d] is %d\n",__LINE__, whichp, parenthesis[whichp]);
                // location_parenopen_column[whichp] = what you need to iterate location_parenopen_line[whichp] = the line you are now on */
    /* program[location_parenopen_line[whichp]][location_parenopen_column[whichp]] = the character where you are right now
...
*/
    ...
    printerrors(parenthesis, maxparenthesis, location_parenopen_line, location_parenopen_column,
                location_parenclose_line, location_parenclose_column, extra_closed_parenthesis, maxextraclosedparenthesis);

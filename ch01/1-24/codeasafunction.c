...
void keeptrackof(char openchar, char closechar, int ifthisis0youclosedit[], int counter,
                 int maxcount, int columnofopenchar[], const char *columnofopenchararrayname, int lineofopenchar[],
                 const char *lineofopenchararrayname, int closecolumn[], const char *closecolumnarrayname, int closeline[],
                 const char *closelinearrayname, int extraclosedtrackingarray[])
{
    if (program[line][column] == openchar) /*if we have an open character*/
    {
        ...
        columnofopenchar[counter] = column; /*store the column coordinate of the nth opened char in the location array*/
        lineofopenchar[counter] = line;     /*store the line coordinate of the nth opened char in the location array*/
        printf("DEBUG %d OPEN %c FOUND: counter is %d, program[%s[%d]][%s[%d]] is %c, program[%d][%d] is %c \n\n",
               __LINE__, openchar, counter, lineofopenchararrayname, lineofopenchar[counter],
               columnofopenchararrayname, columnofopenchar[counter], openchar,
    ...
    else if (program[line][column] == closechar) /*if we have a closed char */
    {
        ...
        printf("DEBUG %d CLOSE %c FOUND: counter is %d, program[%s[%d]][%s[%d]] is %c, program[%d][%d] is %c \n\n",
               __LINE__, closechar, counter, closelinearrayname, closeline[counter],
               closecolumnarrayname, closecolumn[counter], closechar,
               line, column, program[line][column]);
        ...
        for (column = 0; column < howlongisthisline[line]; column++)
        {
            ...
            while ((weare__acomment == OUT) && (areweinastring == OUT))
            {
                ...
                if (program[line][column] == '(') /*if we have an open parenthesis*/
                {
                    ...
                    location_parenopen_column[p] = column;
                    location_parenopen_line[p] = line;
                    printf("DEBUG %d OPEN PARENTHESIS FOUND: p is %d, program[location_parenopen_line[%d]][location_parenopen_column[%d]] is %c, program[%d][%d] is %c \n\n", __LINE__, p, program[location_parenopen_line[p]][location_parenopen_column[p]], p, p, program[line][column], line, column); /*debugoutput*/
                ...
                else if (program[line][column] == ')') /*if we have a closed parenthesis*/
                {
                    ...
                    location_parenopen_column if (p < 0)
                    {
                    ...
                    location_parenclose_column[p] = column; /*register that the nth parenthesis' that's closed column location is column*/
                    location_parenclose_line[p] = line;     /*register that the nth parenthesis' that's closed location is line */

                    printf("DEBUG %d CLOSE PARENTHESIS FOUND: p is %d, program[location_parenclose_line[%d]][location_parenclose_column[%d]] is %c, program[%d][%d] is %c \n\n", __LINE__, p, program[location_parenclose_line[p]][location_parenclose_column[p]], p, p, program[line][column], line, column); /*debugoutput*/
